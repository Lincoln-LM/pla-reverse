...
inline unsigned long rand(struct xoroshiro *rng, const unsigned long maximum,
...
inline unsigned long rand_mask(struct xoroshiro *rng,
                               const unsigned long mask) {
...
inline void find_fixed_seeds_core(const uint offset, __global uint *cnt,
                                  __global ulong *res_g) {
  unsigned int ab = get_global_id(0) + offset;
  ...
  unsigned long vec =
      ((((IVS[0] - a) & 31) << 0) | (a << 5) | (((IVS[1] - b) & 31) << 10) |
       (b << 15) | (((IVS[2] - c) & 31) << 20) | (c << 25) |
       (((IVS[3] - d) & 31) << 30) | (d << 35) | (((IVS[4] - e) & 31) << 40) |
       (e << 45) | (((IVS[5] - f) & 31) << 50) | (f << 55)) ^
      IV_CONST;
...
__kernel void find_fixed_seeds(__global uint *cnt, __global ulong *res_g) {
  find_fixed_seeds_core(0, cnt, res_g);
}
__kernel void find_fixed_seeds_split(__global uint *cnt, __global ulong *res_g,
                                     const int index) {
  find_fixed_seeds_core(index, cnt, res_g);
}